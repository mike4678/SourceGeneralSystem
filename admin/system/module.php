		...
		if ($name && $name != '.' && $name != '..' && filetype($tpath . $name) == 'dir') {
			if (file_exists($tpath . $name . '/config.xml')) {
				...
				if (file_exists($tpath . $name . '/install.json')) {
					$ahtml = "
					<a class='pk-text-success pk-hover-underline' href='javascript:' onclick='pkalert(\"安装该模板后后台将被自动刷新，请保存当前设置，确认继续？\",\"提示\",\"pktip(\\\"正在安装请稍后...\\\",\\\"loading\\\",0);location.href=\\\"admin.php?/system/module/{$name}&ml=install\\\"\")'>安装</a>
					";
				} else { 
					$ahtml = "
...
<div class='pk-row pk-padding-top-15 pk-padding-bottom-15' style='border-bottom:solid 1px #eee'>
	<label class='pk-w-sm-3 pk-text-right' style='padding-top:2px'>
		<img src='../module/{$name}/logo.png' onerror=\"this.src='../module/{$name}/img/template.png'\" style='width:48px;height:48px'>
	</label>
